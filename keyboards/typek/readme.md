...
Handled in `via_command_kb` in `typek.c`; layers outside the dynamic keymap range are ignored, and no reply is sent. Layer 4 (`Game`) exists for this: Caps Lock becomes a plain Right Ctrl instead of the base layer's Ctrl/Esc tap-hold and the bottom-left key becomes Left Ctrl (as on the Windows layer), everything else falls through, and the host turns it on while Valheim has focus. The Linux daemon that drives it is [qmk-autolayer](https://github.com/ksc98/qmk-autolayer): it follows Hyprland focus events and matches the focused window's class or executable against rules in `~/.config/qmk-autolayer/config.toml`. It needs the same hidraw udev rule as VIA above.
